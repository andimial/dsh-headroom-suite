...
var SAVED_BASEURL_PATH = join(PLUGIN_HOME, "saved-baseurl");
...
function isThirdPartyBaseURL(baseURL) {
  return typeof baseURL === "string" && baseURL !== DIRECT_BASE_URL && baseURL !== HEADROOM_BASE_URL;
}
      ...
      handler: async (request, response) => {
        ...
        try {
          ...
          if (target === "headroom") {
            ...
            sendJson(response, 200, { ok: true, savedBaseURL: saved2 ?? null });
            return;
          }
...
async function startProxy(log) {
  ...
  try {
    ...
    child.unref();
    writeFileSync2(startupLogPath(), `${(/* @__PURE__ */ new Date()).toISOString()} spawned headroom proxy pid=${child.pid}
`, { flag: "a" });
