...
- **启动 / 停止代理进程**（detached 启动，不随 dsh 重启而退出；按端口找 PID 停止）；启动时代理按 `llm-deepseek` 线路自动推导上游——第三方地址可作代理上游（见「配置」）
- 实时健康状态（版本 + PID 徽标）
...
> 以上实测数字与「前缀缓存约 1/10 价」仅对 **DeepSeek 官方上游**成立。切到第三方上游时请求内压缩仍生效，但缓存折扣等口径取决于该提供方，不能套用本表数字。
...
「启动代理进程」（面板）与 `/headroom-start`（命令）启动时并不固定转发 DeepSeek——启动瞬间按 `llm-deepseek` 的状态推导上游：

| `llm-deepseek.baseURL` 状态                                            | 代理上游                        |
| -------------------------------------------------------------------- | --------------------------- |
| 第三方地址（第三方直发线路）                                               | 该第三方地址                     |
| 压缩线路 + 保存文件有第三方地址（先录第三方、再切压缩线路的流程）                  | 保存文件中的第三方地址              |
| 其余（官方直连 / 压缩线路且无保存文件）                                       | DeepSeek 官方                 |

- 期望上游为第三方时代理**只服务 OpenAI 协议**（不启用 anthropic 上游参数，不支持 Claude Code）；期望上游为 DeepSeek 时维持现有双端点预设。
- 推导只发生在启动瞬间：已运行的代理不会自动换上游，切换上游请先「停止」再「启动」。
- 手动启动命令不受此规则影响——上游由你显式给的参数决定（见上表命令）。
...
**解决**：显式带 `--openai-api-url https://api.deepseek.com`（见上方推荐启动命令）。通过面板/命令启动时套件按「自动上游选择」填上游，不会落到默认 OpenAI 端点。

> 若启动日志显示上游为推导出的第三方地址且该地址不可达，同样表现为 502：把线路切回直连（清除第三方 baseURL/保存文件），停止并重新启动代理即回到 DeepSeek 官方上游。
...
| Anthropic（Claude Code） | 经 Headroom 压缩（仅 DeepSeek 官方上游） | ✅ 上游支持；第三方上游时禁用 |
